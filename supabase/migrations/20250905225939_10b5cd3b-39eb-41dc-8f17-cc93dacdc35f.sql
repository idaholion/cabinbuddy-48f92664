    ...
    IF checklist_record.items IS NOT NULL AND jsonb_array_length(checklist_record.items) > 0 THEN
      FOR item_record IN 
        SELECT 
          ...
          item->>'marker' as item_marker_name,
...
-- Add unique constraint to prevent duplicate images per organization (only if it doesn't exist)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'checklist_images' 
        AND constraint_name = 'checklist_images_org_url_unique'
    ) THEN
        ALTER TABLE checklist_images 
        ADD CONSTRAINT checklist_images_org_url_unique 
