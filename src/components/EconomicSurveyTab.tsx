...
const EconomicSurveyTab = () => {
  ...
  const columns = [
    ...
    ...['shopped', 'homeRepair', 'dinedOut', 'hiredGuide', 'tickets', 'yellowstone', 'fishingLicense', 'other'].map(key => ({
      key,
      title: categoryLabels[key] || key,
      render: (_: any, row: any) => {
        const amount = row.responses?.[key];
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Accumulated survey data from cabin checkouts — spending in the local area
          </p>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(Object.values(surveyTotals).reduce((sum, val) => sum + val, 0))}
            </div>
            <p className="text-xs text-muted-foreground">Combined spending in area</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Total amounts reported across all survey categories</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {Object.entries(surveyTotals).map(([key, value]) => (
              <div key={key} className="space-y-1">
                <p className="text-sm font-medium text-muted-foreground">{categoryLabels[key] || key}</p>
